...
All CRAN packages have a title, description, package connections (suggests, depends and imports of other packages) and other meta-information in the DESCRIPTION file. We use the text data from the title and description (as accessed in `r as.Date(file.info("_targets/objects/ngram_download_data")$mtime)`) in Section `r ref("topics")`.
...
The list of packages in each CRAN task views (as of `r as.Date(file.info("_targets/objects/ctv_pkgs_data")$mtime)`) are used to contrast the characteristics of DoE packages in Sections `r ref("popular")` and `r ref("silo")`. 
...
There are at least 50 DoE packages since 2013 but most of the downloads are concentrated in just a handful of packages. For example, Figure `r ref("fig:plot-lorenz")` shows a Lorenz curve [@Lorenz1905-tc] for the total package downloads in 2021 of `r tar_read(gini_yearly) %>% filter(year==2021) %>% pull(n)` DoE packages (first released prior to 2021); we can see from Figure `r ref("fig:plot-lorenz")` that bottom 90\% of DoE packages (in terms of total download count in 2021) only share about `r lorenz_value(0.9)`\% of total downloads across all DoE packages -- in another words, `r 100 - lorenz_value(0.9)`\% of the total downloads are due to `r round(npkgs * 0.1)` packages (10% of the DoE packages).
...
The `DoE.base` package [@DoE.base] can construct full factorial and (regular and irregular) orthogonal array designs via `fac.design()` and `oa.design()`, respectively. The `FrF2` package [@FrF2] constructs regular fractional 2-level factorial designs using `FrF2()` and `FrF2Large()` with the latter for large designs.  The `conf.design` package  [@conf.design] constructs symmetric confounded factorial designs via `conf.design()`, and `BHH2` package [@BHH2] generates full or fractional 2-level factorial design matrix via `ffDesMatrix()`. 
