...
The utility of the software can also be described by its design to facilitate the clear expression and interpretation of the desired experimental design. Certain programming language designs can hinder or discourage the development of reliable programs [@Wasserman1975-xr].  The immense popularity of `r CRANpkg("tidyverse")` [a collection of R-packages for various stages of data analysis that places enormous emphasis on the interface design by @Wickham2019-mj] is a testament to the impact that an interface design can have in practice. The practice of experimental design can be advanced by adopting similar interface design principles across the DoE packages. 



The remainder of this paper is organized as follows. `r secref("data", "data")` briefly describes the data source used for the analysis; `r secref("eda", "Exploratory data analysis" )` presents some insights into the state of the current DoE packages by the exploratory data analysis of package download data, text descriptions and comparisons with other CRAN task views; `r secref("design", "Interface design")` discusses the interface designs of widely used DoE packages, and we conclude with a discussion in `r secref("discussion", "Discussion")` of future prospects in the software development of experimental designs. 
...
We can see in Figure `r ref("fig:rank-over-time")` that most of the top 10 ranking packages have been in the top 10 for the last nine years with `r CRANpkg("lhs")` steadily climbing up the ranks in the last few years. It should be noted that the download of one package can prompt the download of another package; the most notable package connection is `r CRANpkg("AlgDesign")` and `r CRANpkg("agricolae")`, where the former is an import for the latter. The full network of package connections within the DoE packages is shown in Figure `r ref("fig:plot-doe-network")`. 
...
2. Multiple mentions of the same bigram within a package were counted as one (for example, `r CRANpkg("AlgDesign")` mentions "experimental design" four times in the title and the description, but this is counted as one). 
3. Bigrams consisting of stop words were removed. The stop words are sourced from the lexicons in `tidytext::stop_words` in addition to other words we deemed irrelevant, e.g. "provide", "e.g.", "calculate" and so on -- full list is shown in the code provided in the link under `r secref("pkgs", "Acknowledgement")`.
...
The `r CRANpkg("DoE.base")` package [@DoE.base] can construct full factorial and (regular and irregular) orthogonal array designs via `fac.design()` and `oa.design()`, respectively. The `r CRANpkg("FrF2")` package [@FrF2] constructs regular fractional 2-level factorial designs using `FrF2()` and `FrF2Large()`, with the latter for large designs.  The `r CRANpkg("conf.design")` package  [@conf.design] constructs symmetric confounded factorial designs via `conf.design()` and the `r CRANpkg("BHH2")` package [@BHH2] generates a full or fractional 2-level factorial design matrix via `ffDesMatrix()`. 
...
This paper uses `r CRANpkg("targets")` [@targets] and `r CRANpkg("renv")` [@renv] for reproducibility, `r CRANpkg("knitr")` [@knitr] and `r CRANpkg("rmarkdown")` [@rmarkdown] for creating reproducible documents, `r CRANpkg("ggplot2")` [@ggplot2], `r CRANpkg("ggraph")` [@ggraph], `r CRANpkg("ggwordcloud")` [@ggwordcloud] and `colorspace` [@colorspace] for visualization, `r CRANpkg("kableExtra")` [@kableExtra] for customizing the table in the Supplementary Material and `r CRANpkg("tidyverse")` [@Wickham2019-mj], `r CRANpkg("tidytext")` [@tidytext], `r CRANpkg("pluralize")` [@pluralize], and `r CRANpkg("ineq")` [@ineq] for data processing and manipulation, and `r CRANpkg("cranlogs")` [@cranlogs] and `r CRANpkg("ctv")` [@ctv] for extracting data. All codes used to reproduce this paper are found in `r url("https://github.com/emitanaka/paper-DoE-review")`.
