...
pkg_nodes <- aggregate(author ~ L2, aut_tidy_join, FUN = toString) %>%
  mutate(note = ifelse(str_detect(author, "Ulrike"), "Groemping",
                       ifelse(str_detect(L2, "Dice"), "Dupuy, Roustant, Richet, Deville",
                              ifelse(str_detect(author, "Gramacy"), "Gramacy, Taddy",
                                     ifelse(str_detect(author, "Shwetank"), "Lall, Vhargese",
                                            ifelse(str_detect(author, "Kevin Wright"), "Wright",
                                                   ifelse(str_detect(author, "Peng Liu"), "Liu",
                                                          ifelse(str_detect(author, "Ernesto"), "Barrios", "Other")))))))) %>%
  mutate(name_add = ifelse(L2 == "plgp", "plgp (G)",
                     ifelse(L2 == "tgp", "tgp (G,T)",
                            ifelse(L2 == "dynaTree", "dynaTree (G,T)",
                                   ifelse(L2 == "DiceDesign", "DiceDesign (Du,Ro)",
                                          ifelse(L2 == "DiceEval", "DiceEval (Du)",
                                                 ifelse(L2 == "DiceKriging", "DiceKriging (Ro;De;Ri)",
                                                        ifelse(L2 == "DiceView", "DiceView (De;Ri)", L2))))))))
...
```{r, fig.width=12, fig.height = 7, fig.cap= "Package network based on first authors who created more than 2 DoE packages. Packages authored by the same person tend to interplay each other."}
