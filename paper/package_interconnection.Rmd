...
# create igraph object
routes_igraph_static <- graph_from_data_frame(d = pkg_edges, vertices = pkg_nodes_static, directed = TRUE)

# create the input based on the igraph object
routes_tidy_static <- routes_igraph_static %>% as_tbl_graph() %>%
  activate(edges)

# create the graph
ggraph(routes_tidy_static, layout = "stress") + 
  geom_edge_link(aes(color = type),
                 arrow = arrow(length = unit(3, 'mm')), 
                 end_cap = circle(3, 'mm')) + 
  geom_node_point(aes(color = first_author), size = 4) + 
  geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  scale_color_manual(values = c("blue", "grey50", "tomato", "magenta")) +
  theme_graph()
```
