...
routes_igraph <- graph_from_data_frame(d = pkg_edges, vertices = pkg_nodes_active, directed = TRUE)
...
```{r network-graph, eval = FALSE, fig.cap = "Interaction network between DoE R packages. The color of edges reflect the type of interaction, i.e., depends, imports, and suggests. Nodes' color indicates wether the package is authored by Ulrike Groumping or not."}
# create the graph
ggraph(routes_tidy, layout = "stress") + 
  geom_edge_link(aes(color = type),
                 arrow = arrow(length = unit(1, 'mm')), 
                 end_cap = circle(1, 'mm')) + 
  geom_node_point(aes(color = first_author)) + 
