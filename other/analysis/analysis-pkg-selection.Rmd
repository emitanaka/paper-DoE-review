...
```{r}
# installation issues
exclude_pkgs <- c("osmgeosample", "secrdesign", "tanaka", "bioOED", "doebioresearch", "ssizeRNA", "SurvGSD", "RcmdrPlugin.SCDA")
design_pkgs <- db %>% 
  ...
  mutate(taskview_doe = ifelse(Package %in% doe_pkgs, "yes", "no"),
         ...
         criteria = case_when(str_detect(tolower(paste(Title, Description)),
                                         "(experiment|block design|factorial design|clinical trial|optimal design|response surface design|stepped wedge design|split-plot design|phase i|crossover design|cluster randomi[sz]ed|non-inferiority trial|supersaturated design|resolvable design|sequential design|sequential enrichment design|space-filling design|latin hypercube|dose[- ]finding|dose-escalation)") ~ "DoE",
                              TRUE ~ "No")) %>% 
...
```{r}
exclude_pkgs2 <- c(
  "augmentedRCBD", # analysis only
  "BayesCTDesign" # power and sample size calcuation only
)
include_pkgs <- c(
...
design_pkgs %>% 
    filter(criteria == "DoE") %>% 
    filter(str_detect(tolower(Description), "sequential|adaptive|sample size")) %>% 
    mutate(fns = map(Package, ~as.character(lsf.str(glue("package:{.x}"))))) %>%
    mutate(func = map2_chr(fns, Package, ~paste0("<div class='tooltip'>", .x, "<span class='tooltiptext'>", getHelpList(.x, .y), "</span></div>", collapse = ", ")))  %>% 
  mutate(Package = glue("<a href='https://cran.r-project.org/web/packages/{Package}/index.html'>{Package}</a>")) %>% 
